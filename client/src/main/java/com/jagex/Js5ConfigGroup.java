package com.jagex;

import deob.ObfuscatedName;

@ObfuscatedName("ed")
public class Js5ConfigGroup {

    @ObfuscatedName("ed.u")
    public static final Js5ConfigGroup FLUTYPE = new Js5ConfigGroup(1);

    @ObfuscatedName("ed.j")
    public static final Js5ConfigGroup HUNTTYPE = new Js5ConfigGroup(2);

    @ObfuscatedName("ed.a")
    public static final Js5ConfigGroup IDKTYPE = new Js5ConfigGroup(3);

    @ObfuscatedName("ed.s")
    public static final Js5ConfigGroup FLOTYPE = new Js5ConfigGroup(4);

    @ObfuscatedName("ed.c")
    public static final Js5ConfigGroup INVTYPE = new Js5ConfigGroup(5);

    @ObfuscatedName("ed.m")
    public static final Js5ConfigGroup LOCTYPE = new Js5ConfigGroup(6, 8);

    @ObfuscatedName("ed.t")
    public static final Js5ConfigGroup MESANIMTYPE = new Js5ConfigGroup(7);

    @ObfuscatedName("ed.l")
    public static final Js5ConfigGroup ENUMTYPE = new Js5ConfigGroup(8, 8);

    @ObfuscatedName("ed.f")
    public static final Js5ConfigGroup NPCTYPE = new Js5ConfigGroup(9, 7);

    @ObfuscatedName("ed.d")
    public static final Js5ConfigGroup OBJTYPE = new Js5ConfigGroup(10, 8);

    @ObfuscatedName("ed.z")
    public static final Js5ConfigGroup PARAMTYPE = new Js5ConfigGroup(11);

    @ObfuscatedName("ed.n")
    public static final Js5ConfigGroup SEQTYPE = new Js5ConfigGroup(12, 7);

    @ObfuscatedName("ed.o")
    public static final Js5ConfigGroup SPOTTYPE = new Js5ConfigGroup(13, 8);

    @ObfuscatedName("ed.q")
    public static final Js5ConfigGroup VARBIT = new Js5ConfigGroup(14, 10);

    @ObfuscatedName("ed.p")
    public static final Js5ConfigGroup VARCSTR = new Js5ConfigGroup(15);

    @ObfuscatedName("ed.w")
    public static final Js5ConfigGroup VARP = new Js5ConfigGroup(16);

    @ObfuscatedName("ed.b")
    public static final Js5ConfigGroup CATEGORY = new Js5ConfigGroup(17);

    @ObfuscatedName("ed.x")
    public static final Js5ConfigGroup AREATYPE = new Js5ConfigGroup(18);

    @ObfuscatedName("ed.i")
    public static final Js5ConfigGroup VARCINT = new Js5ConfigGroup(19);

    @ObfuscatedName("ed.v")
    public static final Js5ConfigGroup VAROBJ = new Js5ConfigGroup(20);

    @ObfuscatedName("ed.k")
    public static final Js5ConfigGroup VARPSTR = new Js5ConfigGroup(21);

    @ObfuscatedName("ed.h")
    public static final Js5ConfigGroup VARS = new Js5ConfigGroup(22);

    @ObfuscatedName("ed.r")
    public static final Js5ConfigGroup VARSSTR = new Js5ConfigGroup(23);

    @ObfuscatedName("ed.g")
    public static final Js5ConfigGroup VARN = new Js5ConfigGroup(24);

    @ObfuscatedName("ed.y")
    public static final Js5ConfigGroup VARNBIT = new Js5ConfigGroup(25);

    @ObfuscatedName("ed.e")
    public static final Js5ConfigGroup STRUCTTYPE = new Js5ConfigGroup(26);

    @ObfuscatedName("ed.ay")
    public static final Js5ConfigGroup CHATPHRASETYPE = new Js5ConfigGroup(27);

    @ObfuscatedName("ed.af")
    public static final Js5ConfigGroup CHATCATTYPE = new Js5ConfigGroup(28);

    @ObfuscatedName("ed.ar")
    public static final Js5ConfigGroup SKYBOXTYPE = new Js5ConfigGroup(29);

    @ObfuscatedName("ed.an")
    public static final Js5ConfigGroup SKYDECORTYPE = new Js5ConfigGroup(30);

    @ObfuscatedName("ed.ap")
    public static final Js5ConfigGroup LIGHTTYPE = new Js5ConfigGroup(31);

    @ObfuscatedName("ed.ag")
    public static final Js5ConfigGroup BASTYPE = new Js5ConfigGroup(32);

    @ObfuscatedName("ed.as")
    public static final Js5ConfigGroup CURSORTYPE = new Js5ConfigGroup(33);

    @ObfuscatedName("ed.ai")
    public static final Js5ConfigGroup MSITYPE = new Js5ConfigGroup(34);

    @ObfuscatedName("ed.ao")
    public static final Js5ConfigGroup QUESTTYPE = new Js5ConfigGroup(35);

    @ObfuscatedName("ed.al")
    public static final Js5ConfigGroup MELTYPE = new Js5ConfigGroup(36);

    @ObfuscatedName("ed.at")
    public static final Js5ConfigGroup field1662 = new Js5ConfigGroup(37);

    @ObfuscatedName("ed.ax")
    public static final Js5ConfigGroup field1663 = new Js5ConfigGroup(38);

    @ObfuscatedName("ed.aw")
    public static final Js5ConfigGroup field1664 = new Js5ConfigGroup(39);

    @ObfuscatedName("ed.aa")
    public static final Js5ConfigGroup DBTABLETYPE = new Js5ConfigGroup(40);

    @ObfuscatedName("ed.am")
    public static final Js5ConfigGroup DBROWTYPE = new Js5ConfigGroup(41);

    @ObfuscatedName("ed.ad")
    public static final Js5ConfigGroup CONTROLLERTYPE = new Js5ConfigGroup(42);

    @ObfuscatedName("ed.az")
    public static final Js5ConfigGroup field1642 = new Js5ConfigGroup(43);

    @ObfuscatedName("ed.ak")
    public static final Js5ConfigGroup field1669 = new Js5ConfigGroup(44);

    @ObfuscatedName("ed.av")
    public static final Js5ConfigGroup field1670 = new Js5ConfigGroup(45);

    @ObfuscatedName("ed.aj")
    public static final Js5ConfigGroup HITMARKTYPE = new Js5ConfigGroup(46);

    @ObfuscatedName("ed.ah")
    public static final Js5ConfigGroup VARCLAN = new Js5ConfigGroup(47);

    @ObfuscatedName("ed.au")
    public static final Js5ConfigGroup ITEMCODETYPE = new Js5ConfigGroup(48);

    @ObfuscatedName("ed.ae")
    public static final Js5ConfigGroup CATEGORYTYPE = new Js5ConfigGroup(49);

    @ObfuscatedName("ed.ac")
    public static final Js5ConfigGroup VARREGION = new Js5ConfigGroup(50);

    @ObfuscatedName("ed.aq")
    public static final Js5ConfigGroup field1639 = new Js5ConfigGroup(51);

    @ObfuscatedName("ed.ab")
    public static final Js5ConfigGroup field1677 = new Js5ConfigGroup(53);

    @ObfuscatedName("ed.bq")
    public static final Js5ConfigGroup VARCLANSETTING = new Js5ConfigGroup(54);

    @ObfuscatedName("ed.bz")
    public static final Js5ConfigGroup GAMELOGEVENT = new Js5ConfigGroup(70);

    @ObfuscatedName("ed.bv")
    public static final Js5ConfigGroup HEADBARTYPE = new Js5ConfigGroup(72);

    @ObfuscatedName("ed.bj")
    public static final Js5ConfigGroup field1681 = new Js5ConfigGroup(73);

    @ObfuscatedName("ed.bf")
    public static final Js5ConfigGroup field1633 = new Js5ConfigGroup(74);

    @ObfuscatedName("ed.bt")
    public final int field1668;

    @ObfuscatedName("ed.bg")
    public final int field1684;

    public Js5ConfigGroup(int arg0) {
        this(arg0, 0);
    }

    public Js5ConfigGroup(int arg0, int arg1) {
        this.field1668 = arg0;
        this.field1684 = arg1;
    }

    @ObfuscatedName("ed.u(B)I")
    public int method3071() {
        return 0x1 << this.field1684;
    }

    @ObfuscatedName("ed.j(II)I")
    public int method3073(int arg0) {
        return arg0 >>> this.field1684;
    }

    @ObfuscatedName("ed.a(II)I")
    public int method3075(int arg0) {
        return arg0 & (0x1 << this.field1684) - 1;
    }
}
